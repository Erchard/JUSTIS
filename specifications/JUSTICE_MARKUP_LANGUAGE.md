...
- рішення;
- статус консенсусу;
- процедурна надійність рішення;
- статус ратифікації;
- dissent note;
- умови перегляду;
- дерева аргументів;
- записи законів, аксіом і презумпцій.

Не обов'язково формалізувати пояснювальну прозу, історичний контекст і довгі філософські коментарі, якщо формалізація не додає якості.
...
| `responsio` | відповідь |
| `sessio` | сесія |
| `deliberatio` | дискусія |
| `decisio` | рішення |
| `actus` | виступ / мовленнєвий акт |
...
| `ratificatio_requiritur` | потребує ратифікації |
| `deliberatio_plena` | повна дискусійна структура |
| `deliberatio_partialis` | часткова дискусійна структура |
| `summarium_provisorium` | попереднє резюме |
| `documentum_provisorium` | попередній документ без окремого протоколу |
...
| `respondet_ad` | відповідає на |
| `status_obiectionis` | статус заперечення |
| `collegium` | колегія учасників |
| `agenda` | порядок денний |
| `quaestio` | питання |
| `probatio_processus` | процедурна надійність |
| `ratificatio` | статус ратифікації |
| `actus` | список мовленнєвих актів |
| `objectiones_apertae` | відкриті заперечення |
